    ...
    <div class="space-y-6 animate-fade-in">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-muted-foreground">Manage patient records</p>
        </div>
        @if (canCreate) {
        ...
        <div class="p-4 pb-3">
          <div class="relative max-w-sm">
            ...
            <input [(ngModel)]="searchTerm" placeholder="Search patients..."
              class="w-full rounded-lg border border-input bg-background pl-9 pr-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b bg-muted/50">
                ...
                <th class="text-left py-3 px-4 font-medium text-muted-foreground">Age</th>
                <th class="text-left py-3 px-4 font-medium text-muted-foreground">Condition</th>
                <th class="text-left py-3 px-4 font-medium text-muted-foreground">Bed</th>
            ...
            <tbody>
              @for (p of filteredPatients(); track p.id) {
                <tr class="border-b hover:bg-muted/50 transition-colors">
                  ...
                  <td class="py-3 px-4">{{ p.age }}</td>
                  <td class="py-3 px-4">
                    <span class="text-xs px-2 py-1 rounded-full border" [class]="conditionClass(p.condition)">
                      {{ p.condition }}
                    </span>
                  </td>
                  <td class="py-3 px-4">{{ p.bed?.bedNumber || 'Unassigned' }}</td>
                  <td class="py-3 px-4">{{ p.dischargeDate ? 'Discharged' : 'Admitted' }}</td>
                    ...
                    <div class="flex items-center justify-end gap-1">
                      @if (canUpdateCondition && p.condition !== 'CRITICAL') {
                        <button (click)="markCritical(p.id!)" title="Mark as Critical"
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>
                        </button>
                      }
                      @if (canAssignBed && !p.bed && !p.dischargeDate) {
                        <button (click)="allocateBed(p.id!)" title="Assign Bed"
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 20v-8a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v8"/><path d="M4 10V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v4"/><path d="M12 4v6"/><path d="M2 18h20"/></svg>
                        </button>
                      }
                      @if (canDischarge && !p.dischargeDate) {
                        <button (click)="discharge(p.id!)" title="Discharge"
                          class="p-2 rounded-lg hover:bg-warning/10 text-warning transition-colors">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>
                        </button>
                      }
                      @if (canDelete) {
      ...
      @if (showDialog()) {
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" (click)="showDialog.set(false)">
          <div class="bg-card rounded-xl shadow-2xl w-full max-w-md p-6 animate-fade-in" (click)="$event.stopPropagation()">
            ...
            <form (ngSubmit)="addPatient()" class="space-y-4">
              <div class="space-y-2">
              ...
              <div class="flex gap-2">
                ...
                <button type="submit"
                  class="flex-1 bg-primary text-primary-foreground py-2 px-4 rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors">Add Patient</button>
              </div>
            </form>
          </div>
        </div>
