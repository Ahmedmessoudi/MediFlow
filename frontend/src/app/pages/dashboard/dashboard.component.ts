    ...
    <div class="space-y-6 animate-fade-in">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        @for (kpi of kpis(); track kpi.title) {
          <div class="bg-card rounded-xl border hover:shadow-md transition-shadow p-5">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm text-muted-foreground">{{ kpi.title }}</p>
                <p class="text-3xl font-bold text-foreground mt-1">{{ kpi.value }}</p>
                <div class="flex items-center gap-1 mt-1">
                  @if (kpi.up) {
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>
                  } @else {
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-destructive" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"/><polyline points="16 17 22 17 22 11"/></svg>
                  }
                  <span class="text-xs" [class]="kpi.up ? 'text-success' : 'text-destructive'">{{ kpi.trend }}</span>
                </div>
              </div>
      ...
      @if (hasFullDashboard) {
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <!-- Line Chart -->
          <div class="lg:col-span-2 bg-card rounded-xl border p-5">
            <h3 class="text-base font-semibold mb-4">Bed Usage Over Time</h3>
            <canvas #lineChart height="260"></canvas>
          </div>

          <!-- Pie Chart -->
          <div class="bg-card rounded-xl border p-5">
            <h3 class="text-base font-semibold mb-4">Bed Distribution</h3>
            <canvas #pieChart height="200"></canvas>
        ...
        this.kpis.set([
          {
            ...
            icon: '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 20v-8a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v8"/><path d="M4 10V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v4"/><path d="M12 4v6"/><path d="M2 18h20"/></svg>',
            trend: '+2%', up: true, color: 'text-primary'
          },
          {
            title: 'Occupied Beds', value: stats.occupiedBeds,
            icon: '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',
            trend: '+5%', up: true, color: 'text-destructive'
          },
          {
            title: 'Available Beds', value: stats.availableBeds,
            icon: '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2"/></svg>',
            trend: '-3%', up: false, color: 'text-success'
          },
          {
            title: 'ICU Usage', value: stats.icuUsagePercent + '%',
            icon: '<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>',
